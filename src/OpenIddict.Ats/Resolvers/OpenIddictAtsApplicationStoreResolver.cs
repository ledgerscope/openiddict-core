...
namespace OpenIddict.Ats
{
    ...
    /// </summary>
    public class OpenIddictAtsApplicationStoreResolver : IOpenIddictApplicationStoreResolver
    {
        private readonly ConcurrentDictionary<Type, Type> _cache = new ConcurrentDictionary<Type, Type>();
        private readonly IServiceProvider _provider;

        public OpenIddictAtsApplicationStoreResolver(IServiceProvider provider)
            => _provider = provider;
        ...
        public IOpenIddictApplicationStore<TApplication> Get<TApplication>() where TApplication : class
        {
            ...
            {
                if (!typeof(OpenIddictAtsApplication).IsAssignableFrom(key))
                {
                    throw new InvalidOperationException(SR.GetResourceString(SR.ID0257));
                }

                return typeof(OpenIddictAtsApplicationStore<>).MakeGenericType(key);
            });
