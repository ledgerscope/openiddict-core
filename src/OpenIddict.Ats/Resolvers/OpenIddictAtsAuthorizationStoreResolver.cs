...
namespace OpenIddict.Ats
{
    ...
    /// </summary>
    public class OpenIddictAtsAuthorizationStoreResolver : IOpenIddictAuthorizationStoreResolver
    {
        private readonly ConcurrentDictionary<Type, Type> _cache = new ConcurrentDictionary<Type, Type>();
        private readonly IServiceProvider _provider;

        public OpenIddictAtsAuthorizationStoreResolver(IServiceProvider provider)
            => _provider = provider;
        ...
        public IOpenIddictAuthorizationStore<TAuthorization> Get<TAuthorization>() where TAuthorization : class
        {
            ...
            {
                if (!typeof(OpenIddictAtsAuthorization).IsAssignableFrom(key))
                {
                    throw new InvalidOperationException(SR.GetResourceString(SR.ID0258));
                }

                return typeof(OpenIddictAtsAuthorizationStore<>).MakeGenericType(key);
            });
