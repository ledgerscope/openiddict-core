...
namespace OpenIddict.Ats
{
    ...
    /// </summary>
    public class OpenIddictAtsTokenStoreResolver : IOpenIddictTokenStoreResolver
    {
        private readonly ConcurrentDictionary<Type, Type> _cache = new ConcurrentDictionary<Type, Type>();
        private readonly IServiceProvider _provider;

        public OpenIddictAtsTokenStoreResolver(IServiceProvider provider)
            => _provider = provider;
        ...
        public IOpenIddictTokenStore<TToken> Get<TToken>() where TToken : class
        {
            ...
            {
                if (!typeof(OpenIddictAtsToken).IsAssignableFrom(key))
                {
                    throw new InvalidOperationException(SR.GetResourceString(SR.ID0260));
                }

                return typeof(OpenIddictAtsTokenStore<>).MakeGenericType(key);
            });
